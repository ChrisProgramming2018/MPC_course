...
class ReplayBuffer(object):
    ...
    def __init__(self, obs_shape, action_shape, capacity, seed, device):
        ...
        self.device = device
        self.obses = np.empty((capacity, *obs_shape), dtype=np.float)
        self.next_obses = np.empty((capacity, *obs_shape), dtype=np.float)
        self.actions = np.empty((capacity, *action_shape), dtype=np.float64)
